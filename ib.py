...
# }
# data = Strategy.objects.filter(source='PORTFOLIO VISUALIZER')
# print(data.values())
count = 0
# strats = {}
# if data:
#     for i in data:
#         count+=1
#         strats.update({str(count):list([i.name, i.name])})
# else:
#     pass
...
class pvScraper(webDriver):
    ...
    def getReports(self):
        log.info('PV signin')
        os.system("cd /home/abc/python/working/pv-rebalance/clientportal.gw; bin/run.sh root/conf.yaml")
        print("start open")
        time.sleep(25)
        self.driver.get("https://localhost:5000/sso/Login?forwardTo=22&RL=1&ip2loc=US")
        print("done")
        elem=self.driver.find_element_by_css_selector('#user_name')
        print("Username set")
        elem.send_keys("jsbha122")
        print("Password set")
        elem=self.driver.find_element_by_css_selector('#password')
        elem.send_keys("python123")
        elem.send_keys(Keys.RETURN)
        print("Goiing to seleep")
        time.sleep(40)
        print("Try to check status")
        login_status = self.driver.find_element('tag name', 'pre').text
        match = fuzz.ratio(login_status, 'Client login succeeds')
        match = 100
        if match > 90:
            print("Try to access apis")
            
            url = 'https://localhost:5000/v1/portal/iserver/secdef/search'
            # myobj = {'somekey': 'somevalue'}
            # custom_header = {"content-type": "Custom header content"}
            custom_header = {'Content-Type': 'application/json;charset=utf-8','Keep-Alive': 'timeout=5, max=200'}
            targets = Target.objects.all()
            for target in targets:
                payload = json.dumps({"symbol": target.ticker, "name": True, "secType": "STK"})
                x = requests.post(url, data = payload, verify=False, headers=custom_header)
                # x = requests.post(url, data = payload)
                res = json.loads(x.text)
                # con_id = x[0].get('conid')
                # con_id = res[0].get('conid')
                market_data_url = "https://localhost:5000/v1/portal/iserver/marketdata/history?conid={}&period=1d&bar=1h".format(con_id)
                
                market_data = requests.get(market_data_url, verify=False, headers=custom_header)

                market_data = json.loads(market_data.text)
                try:
                    target.last_fetched_price = market_data.get('data')[0].get('c')
                except Exception as ex:
                    print(str(ex))
                    pass
                # target.con_id = con_id
                target.save()
                print("done")
                # search = dict()
        else:
